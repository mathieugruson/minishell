...
/*   Updated: 2022/12/09 21:47:20 by mgruson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*new_env_var(char *str, char **envp)
{
	...
	while (str[i.i])
	{
		...
		{
			printf("len : %i\n", ft_intlen(exit_status));
			str = get_status(str, (i.i + ft_intlen(exit_status) + 1), (i.i + 1), ft_itoa(exit_status)); // "2" a remplacer par la variable status
