...
/*   Updated: 2022/11/23 18:45:18 by mgruson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**fill_args(char *s, char c, char **s1)
{
	...
	while (s[i.i] && s[i.i] != '\n')
	{
		...
		if (i.l != 0 && i.t++ > -1)
			i.l = 0;
		while (s[i.i] && ((s[i.i] != c && s[i.i] != '|' && s[i.i] != '<' \
		&& s[i.i] != '>' && s[i.i] != '\n') || is_in_quote(s, i.i)))
		{
			s1[i.t][i.l++] = s[i.i++];
		}
...
int	double_pointer_nbr(char *s, char c)
{
	...
	while (s[i.i])
	{
		while ((s[i.i] && (((s[i.i] != c && s[i.i] != '|' && \
		s[i.i] != '>' && s[i.i] != '<')) || is_in_quote(s, i.i))))
		{
			if (i.k++ == 0)
				i.count++;
			i.i++;
...
char	**simple_pointer_nbr(char *s, char c, char **s1)
{
	...
	while (s[i.i] && s[i.i] != '\n')
	{
		while (s[i.i] && s[i.i] == c)
			i.i++;
		while ((s[i.i] == '>' || s[i.i] == '<' || s[i.i] == '|') && i.k++ > -1)
			i.i++;
		if (i.k != 0 && i.t++ > -2)
		{	
			s1[i.t] = malloc_simple_pointer(i.k, i.t, s1);
			i.k = 0;
		}
		while (s[i.i] && ((s[i.i] != c && s[i.i] != '|' && s[i.i] != '<' && \
		s[i.i] != '>' && s[i.i] != '\n') || is_in_quote(s, i.i)) && i.k++ > -1)
			i.i++;
		if (i.k != 0 && i.t++ > -2)
		{
			s1[i.t] = malloc_simple_pointer(i.k, i.t, s1);
